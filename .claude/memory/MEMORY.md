...
- **Release staged (owner approved 2026-07-02): mix.exs both → 0.5.0, dated CHANGELOG section, banners; preflight PASSED for v0.5.0; all on PR #330** — a green squash-merge auto-tags v0.5.0 and closes the 10 milestone issues. Final umbrella: 2,543 tests + 210 properties, 0 failures (one analyzer_interpolation "EXIT killed" flake seen, passes on rerun — known). **Merged + released 2026-07-02** (owner enabled auto-merge; tag v0.5.0 cut). Post-merge follow-ups landed on PR #331: milestones.json closes v0.4.0 + v0.5.0 (both were still "open" — the sync would have REOPENED them), CLAUDE.md narrative synced, and the ci.yml dogfood job's external half falls back to `secrets.GH_PROJECTS_TOKEN` (owner 2026-07-02: the projects PAT has repo read on both private ports — verified 200 on both via API; a separate DOGFOOD_GITHUB_TOKEN is unnecessary; the PAT canNOT administer Skein Actions secrets, 403 on public-key, so the secret must exist repo-side under one of the two names). Pins did NOT need bumping: both port PRs were true merges (not squashes) — skein-testing#36 merged into its prelaunch branch, FablePool#5 into main — so the pinned revs stay reachable. Still open for owner: decide rc.5-vs-rename for the Wave F gate (rc.2–rc.4 tags already exist).
