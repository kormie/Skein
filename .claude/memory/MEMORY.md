...
## CLI TUI / Raxol Spike (#171 — 2026-06-12 session, PR #239)
- **Decision (owner-ratified): Raxol-first**, omnibus `{:raxol, "~> 2.4.0", runtime: false}` in skein_cli; TermUI = named fallback (both TEA; only view ports). Gates: ≤+25% size / ≤+400ms cold start / zero new CI toolchain / byte-identical non-TTY. Measured: 23.04MB (+16.9% vs 19.7MB), ~227ms warm, `raxol_started: false` on plain paths, `driver_backend: :io_terminal`. Upstream engagement deferred until G1 closes (owner choice); 3 draft candidates: vendored-deps packaging fix, Burrito make_cwd, IOTerminal/skip-NIF config.
- **Raxol facts**: TEA runtime + ALL widgets live ONLY in omnibus raxol (Phoenix stack = 27 required deps); raxol_core=telemetry-only, raxol_terminal=ANSI/ScreenBuffer/drivers. Driver fallback is REAL: termbox2 .so never reaches priv/ → `Raxol.Terminal.Driver.backend()` = :io_terminal (pure Elixir, OTP 28 raw mode) on every target — NO native code added. `Raxol.run(app, opts)` returns {:ok, pid} (monitor for DOWN); init/1 gets %{width, height, options: opts}; `use Raxol.Core.Runtime.Application` imports view DSL (column/row/box/text) + key_match + command(:quit).
- **raxol_terminal 2.4.0 packaging defects** (worked around in root mix.exs release step `prune_vendored_raxol_tooling/1`): (a) hex pkg vendors termbox2_nif subproject incl. its elixir_make dep SOURCES under lib/ → modules compile into raxol_terminal.app → "Duplicated modules" kills mix release (prune ebin+.app+struct pre-assemble); (b) Burrito RecompileNIFs runs `make clean all` at DEP ROOT (ignores elixir_make make_cwd) → no-op root Makefile written into deps/raxol_terminal.
- **Release :load pattern**: dep `runtime: false` + explicit release applications entry — a :permanent app may NOT depend on a :load app, runtime:false removes the edge. The whole raxol closure (35 apps incl. mnesia/os_mon/ssh OTP extras) is :load via `tui_applications()` in root mix.exs; TUI entry does ensure_all_started(:raxol); os_mon cpu/mem/disk sup disabled via put_env pre-start (port shutdown noise otherwise).
- **GOTCHA `.mise.toml` [env] MIX_ENV**: forced "dev" OVERRODE explicit MIX_ENV=prod through mise shims — local prod release builds silently became dev (97MB binary). REMOVED in PR #239; CI unaffected (setup-beam, not mise).
- **GOTCHA Burrito extraction cache**: `~/.local/share/.burrito/skein_erts-X_<version>` keyed by version — same-version rebuilds reuse STALE payloads ("Unknown command" for new subcommands). `rm -rf ~/.local/share/.burrito` or `<bin> maintenance uninstall`. Also: BEAM literal chunks are compressed — `strings beam | grep` CANNOT verify literals.
- **Fixed en route**: `skein trace` KeyError'd on ANY real span (printed span.name/span.duration_ms; spans have duration_us, no :name) — now pure `Skein.CLI.Render.trace_plain/1` + `span_line/1` (per-kind: effect/annotation/user_event/state_change, ASCII-only), golden tests pin bytes. TTY gate: `Skein.CLI.Tui.interactive?/2` (--interactive opt-in, --no-tui/SKEIN_NO_TUI off, prim_tty.isatty).
- **TTY wart (Phase 2)**: on a real TTY the release boots an Erlang shell before the TUI (banner flash + "Shell process terminated"); fix = -noshell vm.args. PTY testing trick: `(sleep 2; printf 'q') | script -qec "<bin> trace --interactive" /tmp/cap.txt`.
- **Phase 2 next**: live tail needs a writer half — `skein run --persist-events` (WAL SQLite .skein/events.db, EventStore.append hook, SqliteBackend.persist + rowid-cursor load_since) + SpanSource behaviour; CLI can't read the server's ETS.
- macOS checklist on #171 (first-ever macOS runtime verification, incl. pre-existing exqlite-via-zig question); 4-target dispatch build needs owner click (workflow_dispatch 403 for integration token).
