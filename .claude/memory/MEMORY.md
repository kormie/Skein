...
## Project State (rc.5 SOAK WEEK — 2026-07-03 session; soak bar MET, GA promotion stays owner-gated)
- **v1.0.0-rc.5 released 2026-07-03** (tag on main, 10 assets). This session = soak: flake elimination, external dogfood on the RELEASED artifact, backlog capture, gate polish. **No GA work started — promotion to v1.0.0 is the owner's explicit call.**
- **#338 FIXED + merged (PR #343, squash 93d14e0), re-milestoned into rc.5.** Three classes, each root-caused: (1) registry-emptiness flakes were NOT races — leftover app-global state (ServerTest mounts a module subscribing topic "events", never unsubscribed) + randomized in-module order; Topic/Queue/Schedule/Timer suites now reset BEFORE each test too. (2) all 8 `Enum.random` port files → `Skein.Runtime.TestPorts` (test_helper.exs): Bandit `port: 0` + `ThousandIsland.listener_info(bandit_pid)` (Bandit returns the TI pid); for `Skein.Runtime.Server` use `:sys.get_state(server).bandit_pid` — zero product changes. (3) analyzer_interpolation "EXIT killed" ROOT-CAUSED with a probe: on OTP 28 the THIRD `:code.load_binary` of one module name succeeds and purge-KILLS any process still executing v1 code; ~20 async suites shared `module M`, exactly 4 also RUN compiled code (interpolation/unresolved/line_termination/record_literal) → file-unique names (InterpSegM/UnresolvedM/LineTermM/RecordLitM). **RULE: any async suite that executes compiled Skein code must use a file-unique Skein module name** (documented in each moduledoc). Repro attempt honest: solo x500 + 15 full runs green; the probe demonstrates the mechanism deterministically.
- **External dogfood: BOTH PORTS GREEN ON THE RELEASED rc.5 ARTIFACT (the soak bar — MET).** Binary sha256-verified from the GitHub release, installed over the stale rc.4 at /root/.local/bin/skein (Burrito self-upgrades). skein-testing (dungeon-master): 5/5 suite; live pass with a temp `[env.live.llm] backend="anthropic"` appended to skein.toml (`skein run . --env live`) — two live llm.chat narrations, typed-store put/get round-trips (exercises the #340 @primary fix), 400/404 paths correct; toml restored, .skein/ removed. FablePool: 18/18 suite; live service pass — /demo full convergence pipeline all-invariants-true, /ingest (`{source, content}` shape!), /claims /audit /export /log correct; NO llm.* calls exist in fablepool.skein (canonicalization-stub port — its "live" pass is the running service). **Zero findings filed; zero port changes; dogfood.json pins untouched.**
- **Backlog filed into v1.1 + ROADMAP-linked (PR #344): #341** machine-checkable documentation evidence (evidence-class tags + CI drift gate; successor to 3-releases-running docs sweeps; coordinate with #202), **#342** agent-safe edit protocol / context packs (tooling+MCP, explicitly NOT language syntax).
- **Gate polish (PR #344):** release-readiness.yml test job gained named standalone steps — compiler `test/skein/freeze` (20), runtime event_store_freeze_test, CLI `test/cli/cli_surface_freeze_test.exs` + `dogfood_pins_freeze_test.exs` (CLI freeze tests live at test/cli/, NOT test/skein/cli/), bench replay `mix skein.bench -- --report ...` (alias exists at umbrella ROOT only). **Live bench run: 12/12 green, 9/12 first-try, mean 1.17** — trend holds vs release day; recordings/history/chart refreshed on #344, replay-vs-new-recordings verified deterministic. **workflow_dispatch is 403 for BOTH the GitHub MCP integration and GH_PROJECTS_TOKEN** — on-demand Actions runs (writability-bench.yml, readiness) are owner-only buttons; local `mix skein.bench -- --live` with the extracted key is the working substitute.
- Session gotchas: `pkill -f`/`pgrep -f` patterns match the calling bash -c line itself → shell suicides with exit 144 (kill by pid from pgrep OUTPUT); mise shims are dir-scoped (elixir NOT on PATH outside /home/user/Skein); `mix test | grep` exit is grep's — use `set -o pipefail` + `${PIPESTATUS[0]}`; container locale is latin1 (cosmetic BEAM warning; ELIXIR_ERL_OPTIONS=+fnu or LANG=C.UTF-8 in the CCR env config would silence it — flagged to owner).
- **GA posture after soak day 1: bar met so far** (ports green on released artifact, flakes eliminated at root, benchmark trend flat-good, milestone empty). Remaining soak evidence = a week of green mains with no re-kick commits (#338's practical bar). v1.0.0 promotion NOT staged.
