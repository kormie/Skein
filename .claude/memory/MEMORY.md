...
- **NEXT: /release-readiness 1.0.0-rc.5 verdict** (was running at session close-out — fix any content blockers on a fresh branch/PR; version-staging gate expected NO_GO until the owner approves the bump), then ASK owner re rc.5 bump via bump-version. Spec §6.2 header may still say `capability store.table(name)` (pre-C5 single-arg form) — was flagged mid-session as a possible docs drift for the sweep to confirm.
