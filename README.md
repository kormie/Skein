...
skein version             # → skein 1.0.0-rc.5
...
> **Release posture: Skein is pre-1.0.** The current package version is **`1.0.0-rc.5`**; the last pre-RC development release was **`0.5.0`** — "Runtime Contract & Dogfood" (shipped 2026-07-02, same day as v0.4.0 "Truth & Soundness"): the runtime effect/schema/store/EventStore contracts match the spec — one authoritative effect-ABI registry, a frozen structured-error ABI, one recursive schema engine, typed store tables, EventStore persistence, real OTP supervision, and a continuous dogfood gate. The Wave F freeze then landed for the RC gate: every stable surface — grammar/keywords, the diagnostics registry, the full effect ABI, schema derivation, the CLI/config surface, compiled metadata, persisted EventStore shapes — is declared frozen with executable drift gates (`conformance/freeze/`), and agent-writability is measured, not asserted. The release train from here is **v1.0.0-rc.5 (the true RC) → v1.0.0 GA after the candidate soaks — and GA is not imminent.** The path to a sound, honest, dogfood-proven 1.0 is the contract-first wave plan in [docs/ROADMAP.md](docs/ROADMAP.md).

The compilation pipeline was built in phases — the *pipeline* is complete end-to-end — and the 1.0 stable surfaces are frozen behind executable gates; GA still waits on the RC soak and release mechanics. [docs/ROADMAP.md](docs/ROADMAP.md) tracks what's next:
