    ...
    test "full memory lifecycle: put, get, delete, list" do
      mod =
        ...
        module MemoryService {
          ...
          fn store(key: String, value: String) -> Result[String, MemoryError] {
            memory.put(key, value)
          }

          fn fetch(key: String) -> Result[String, MemoryError] {
            memory.get(key)
          }
    ...
    test "llm.json returns parsed map" do
      mod =
        ...
        module AiJson {
          ...
          fn decide(ticket: String) -> Result[String, LlmError] {
    ...
    test "module compiles and uses both memory and llm" do
      mod =
        ...
        module DecisionService {
          ...
          fn analyze(ticket: String) -> Result[String, LlmError] {
            llm.chat("claude-sonnet-4-5", "Analyze this ticket.", ticket)
          }

          fn save_result(key: String, value: String) -> Result[String, MemoryError] {
            memory.put(key, value)
          }

          fn get_result(key: String) -> Result[String, MemoryError] {
            memory.get(key)
          }
        }
        """)
    ...
    test "llm.json[T] compiles and passes schema derived from type T" do
      mod =
        ...
        module AiTyped {
          ...
          fn decide(ticket: String) -> Result[RefundDecision, LlmError] {
    ...
    test "llm.embed compiles and runs end-to-end" do
      ...
      {:module, mod} =
        ...
        module EmbedIntegration {
          ...
          fn get_embedding(text: String) -> Result[List[Float], LlmError] {
    ...
    test "llm.embed uses same model capability as chat" do
      ...
      {:module, mod} =
        ...
        module EmbedCaps {
          ...
          fn embed_it(text: String) -> Result[List[Float], LlmError] {
            llm.embed("text-embedding-3-small", text)
          }
        }
        """)
