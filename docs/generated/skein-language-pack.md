...
This document is the single-file specification for Skein. It is designed to fit within an LLM context window alongside task-specific code and instructions. The generated [Skein language pack](generated/skein-language-pack.md) is the agent-loadable companion artifact that bundles this spec with frozen registries, canonical compiling examples, and CLI commands under an executable context-budget gate.
