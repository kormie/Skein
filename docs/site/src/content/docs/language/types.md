...
| E0021 | Non-exhaustive match on a boolean (error) |
| E0024 | Unknown type name (error); non-exhaustive match on an enum, Result, or Option (error) |
| E0025 | Invalid constraint annotation |

### Match Exhaustiveness

A match on a closed type (Bool, enums, Result, Option) must cover every case. A missing arm is a **compile error**, not a warning — this code does not compile:

```skein
fn check(x: Bool) -> String {
  match x {
    true -> "yes"
    -- error E0021: Non-exhaustive match: missing pattern 'false'
  }
}
```

Add a `false -> ...` arm or a wildcard `_` pattern to make it compile.
...
The analyzer checks that all enum variants are covered in a match expression (a missing variant is error E0024). A wildcard `_` arm satisfies this check.
