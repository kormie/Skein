...
The analyzer performs four passes over the AST:
...
3. **Pass 3: Capability checking** -- walks function bodies to find effect calls (`http.*`, `store.*`, `memory.*`, `llm.*`), verifies each has a covering capability declaration, and reports missing capabilities (E0030) with `fix_code` for agent auto-fix
4. **Pass 4: Transition checking** -- validates agent `Phase` enum transitions, ensuring `transition(Phase)` calls reference valid target phases reachable from the current phase (E0040)
