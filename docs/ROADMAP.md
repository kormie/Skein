...
- [#319](https://github.com/kormie/Skein/issues/319) — wire or cut the dead declared surfaces *(added 2026-07-02: `supervisor` compiles to `__supervisors__/0` metadata nothing consumes; tool `policy` blocks parse and drop — silently-inert surface teaches agents a false model; lean: cut `policy`, decide `supervisor` on dogfood evidence)*.
