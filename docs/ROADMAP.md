...
- [#262](https://github.com/kormie/Skein/issues/262) — the gate must **compile + load + run** the dogfood programs plus a **negative-fixture corpus** (what would have caught #259); defines the FablePool 1.0 conformance suite (test vectors). *Negative corpus landed (PR #266); positive/executing half pending Waves 3–4 features.* [#202](https://github.com/kormie/Skein/issues/202) — drift guard, broadened to execute. — L
- ~~[#261](https://github.com/kormie/Skein/issues/261)~~ — **done:** pre-freeze policy decisions. Non-exhaustive match on a closed type (Bool/enum/Result/Option) is now a compile error; `Uuid.new`/`Instant.now` are capability-gated, replay-controllable **effects** (`uuid.new()`/`instant.now()`), not ambient stdlib (swift-dependencies model); `resume` documented as reserved-for-1.x; timer surface confirmed accurate. Resolves spec-lies [skein-testing#20](https://github.com/kormie/skein-testing/issues/20) (`resume`) and [#18](https://github.com/kormie/skein-testing/issues/18) (timer).
- **Capability-bound implementations** *(GA blocker, freeze-sensitive; design: `docs/design/capability-bound-implementations.md`)* — the effect override plane reframed: an override is a `capability` with a bound impl (`scenario { capability X via &stub }`), retiring the `Dependencies`/`with_overrides` plane and the "dependency" term. Split: [#267](https://github.com/kormie/Skein/issues/267) (capability-bound impls + runtime, behavioral effects), [#273](https://github.com/kormie/Skein/issues/273) (analyzer-enforced **test purity** — effects only in `scenario`), [#274](https://github.com/kormie/Skein/issues/274) (stub-contract types), [#275](https://github.com/kormie/Skein/issues/275) (**discovery spike**: stateful effects under `scenario`+`golden`, fate of `given`).
- [#268](https://github.com/kormie/Skein/issues/268) — pre-freeze polish: standardize `!`/`?` position in docs/examples; remove vestigial `get!`/`put!`.
- Remaining spec-lie: [skein-testing#9](https://github.com/kormie/skein-testing/issues/9) (agent reachability + docs).
