...
- ~~[#283](https://github.com/kormie/Skein/issues/283)~~ — **done:** `Skein.Runtime.TestPolicy` is the conservative effect policy `skein test` installs around each scenario/golden — deterministic `uuid` (incrementing) / `instant` (stepping) defaults, `model` defaults to a deterministic test backend, `http.out`/live-`model` **blocked** (raising `Skein.Runtime.LiveEffectError`, which fails the test rather than returning a swallowable `Err`), and scenario-local `store`/`memory`/`event.log` reset before each test. Opt back into live with the repeatable `skein test --allow-live <effect>[:<scope>]` (effect ∈ `http.out`/`model`/`uuid`/`instant`; host-scoped for `http.out`). Golden bodies run under replay (Wave 1), so an exhausted/mismatched trace is a structured error, never a live call. Production (`skein run`) is untouched.
- **Remaining (follow-ups):** ~~[#282](https://github.com/kormie/Skein/issues/282) context propagation to spawned processes/tasks/timers~~ — **done** (`Skein.Runtime.SpawnContext` captures the capability stack, registered scenario envelopes, and test policy in the spawning/scheduling process and reinstalls them in the spawned `process.spawn`/`timer` body; `Replay` is intentionally not propagated — its consume-on-read cursor can't be safely copied to a concurrent task); `llm.embed` provider support. ~~#284 documented JSON output schemas for check/test/trace~~ — **done** (see Wave 4).

**Wave 4 — CLI observability & agent contract** *(1.0 blocker)*:
- ~~[#284](https://github.com/kormie/Skein/issues/284)~~ — **done (the framework-neutral split of [PR #239](https://github.com/kormie/Skein/pull/239)):** pure byte-stable `Skein.CLI.Render` + the `skein trace` crash fix + golden trace tests (shipped #284-core, PR #284); the TTY/TUI seam (`--interactive` opt-in, `--no-tui`/`SKEIN_NO_TUI` force plain; MCP/LSP/non-TTY never route through a TUI); and the versioned `--json` agent contract for `compile`/`build`/`test`/`trace` (`Skein.CLI.Json`, PR #287) — one stable envelope `{schema: "skein.<cmd>/v1", ok, data}` with `ok` mirrored into the exit code, documented in `docs/site/.../reference/json-output.md`.
- **Remaining (separately gated):** the **Raxol TUI** ([#171](https://github.com/kormie/Skein/issues/171)) **may land in 1.0 (Wave 4) if it passes its gates** (binary size ≤ +25%, cold-start delta ≤ +400 ms, no NIF on any target, clean teardown, four-target CI, macOS verification) — otherwise it slips to v1.2. It must never route MCP/LSP/non-TTY output through the TUI. The framework-neutral render/JSON work (#284) above is the 1.0 blocker and is **not** coupled to the TUI.
...
- [#171](https://github.com/kormie/Skein/issues/171) — Raxol CLI TUI — **targeted at v0.5.0 (Wave 4) if its gates pass**, falling back here (v1.2) otherwise; separately gated from the framework-neutral CLI render/JSON work (#284, size/cold-start/no-NIF/teardown gates per PR #239) — XL
