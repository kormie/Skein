...
- **Remaining (follow-ups):** [#282](https://github.com/kormie/Skein/issues/282) context propagation to spawned processes/tasks/timers (`CapabilityStack`/`TestPolicy` `snapshot`/`restore` exist for the hand-off); `llm.embed` provider support. ~~#284 documented JSON output schemas for check/test/trace~~ — **done** (see Wave 4).
