...
|---|---|---|
| **v0.4.0 — Truth & Soundness** | Wave A (truth reset) + Wave B (analyzer/codegen soundness, B1–B6 — **complete 2026-07-01**) + Wave B residue (#309 #310 #311 #313) | active |
...
**Wave B closed the soundness half (verified 2026-07-02).** Analyzer/codegen soundness is now established and adversarially probed: `?` truly early-returns (B1), `:unknown`/widened types cannot cross declared boundaries (B2, E0037), call arguments are type-checked for local/effect/callback calls in fn, handler, agent-handler, and tool-implement bodies alike (B3), the analyzer-accept ⇒ Core-gen ⇒ BEAM-compile ⇒ load bridge holds with codegen fallbacks turned into invariant raises plus a property gate (B4), records are nominal and `Option` is total across construction/JSON/store/tool boundaries (B5), and tool/provider bodies are checked against their declared contracts with transitive purity (B6, E0038). Residual holes found by the 2026-07-02 sanity check, filed as v0.4.0 blockers: bare `Ok`/`Err` as a value compiles silently to an atom ([#309](https://github.com/kormie/Skein/issues/309)); interpolation segment types are unchecked, so accepted programs crash at runtime with `:unsupported_interpolation` ([#310](https://github.com/kormie/Skein/issues/310)); the newline-`(` juxtaposition parse misleads agents ([#311](https://github.com/kormie/Skein/issues/311)); E0020's fallback `fix_code` is a `//` placeholder ([#313](https://github.com/kormie/Skein/issues/313)).

**The runtime contract remains drifted (all C-premises re-verified live 2026-07-02):** the effect ABI has three hand-maintained copies that disagree (`timer.cancel` — a `timer.cancel(ref)!` program compiles and loads against a `Result` the runtime never returns), `llm.json[T]` does not validate, LLM/tool/provider errors leak raw Elixir structs that cannot be pattern-matched (probe: an `Err(LlmError.RateLimit(d))` arm compiles but can never match), the typed store is dead code, and the SQLite EventStore is an orphaned backend whose durability docs are false.
...
- [#312](https://github.com/kormie/Skein/issues/312) — Reconcile diagnostic-code ownership: **E0028/E0029 are implemented** but **missing from the §7 registry** (jumps E0027→E0030) *and* from the docs-site errors page. *(README posture/count already corrected — verified 2026-07-02.)*
...
- `#268` — `!`/`?` position standardization + **outright deletion** of `get!`/`put!` (scope sharpened 2026-07-02: the forms stop lexing; no deprecation window — one-way-to-do-things honesty).
...
**Wave B residue** *(found by the 2026-07-02 adversarial probes; the "silently cross" half of the invariant is PARTIAL until these close)* — milestone v0.4.0:
- [#309](https://github.com/kormie/Skein/issues/309) — bare `Ok`/`Err` as a value is accepted with zero diagnostics and lowers to a bare atom (`analyzer.ex:2205-2206`); only fn-return boundaries catch it.
- [#310](https://github.com/kormie/Skein/issues/310) — interpolation segment types unchecked: `"${h}"` for a fn-ref/record/map compiles, loads, then crashes `{:unsupported_interpolation, v}` (`core_erlang.ex:2810-2824`).
- [#311](https://github.com/kormie/Skein/issues/311) — a line starting with `(` parses as a call of the previous expression (`parser.ex:2283`); now a structured E0020 but the diagnosis misleads (decide the grammar before freeze).
- [#313](https://github.com/kormie/Skein/issues/313) — E0020 fallback `fix_code` is a `//` placeholder comment, not applicable Skein (`analyzer.ex:657-661`).
- [#318](https://github.com/kormie/Skein/issues/318) — expression-termination grammar audit *(added 2026-07-02: generalizes #311 — enumerate and spec-pin the newline continuation rule for every postfix/infix production; grammar answers are the most freeze-sensitive surface)*.
- [#319](https://github.com/kormie/Skein/issues/319) — dead declared surfaces *(rescoped 2026-07-02, owner decision: supervision is core to the agent thesis)*: tool `policy` blocks are **cut** (grammar/AST/spec removed; parsing one is a structured error); `supervisor` **stays** — its declaration contract (child spec, `strategy`, `max_restarts n per m s`) is pinned in spec §3.9 as the frozen surface, docs keep saying "metadata only", and the runtime wiring is [#325](https://github.com/kormie/Skein/issues/325) (v0.5.0, below).
...
- **C4 — scenario-provider soundness & capability authority:** the runtime dynamic capability stack, resolution order (`implement → replay → test-default → live → failure`), `LiveEffectError`, and `SpawnContext` propagation **landed and are contract-met** (`capability_stack.ex`, `nondeterminism.ex`, `spawn_context.ex`); `Dependencies`/`with_overrides` are retired. *(Corrected 2026-07-02:)* the analyzer half — provider contract type-checking (E0038) + transitive purity — **landed in B6/#295**. [#279](https://github.com/kormie/Skein/issues/279) is narrowed to the verified remainder: `llm.embed` provider support and the `given`-grammar reconciliation (spec still lists `given_block`, `SKEIN_SPEC.md:292-296`); recommended P0→P1.
- **C5 — honest store contract (direction pre-committed 2026-07-02: TYPED tables):** the active store is **dynamic and untyped** — the analyzer does not know table `T` (`analyzer.ex:163-168`), the primary key is not Uuid-checked, and the Ecto typed-table path (`ecto_schema.ex`, `store_ecto.ex`, `migration_gen.ex`) is **dead code** with no production callers and no Repo in the supervision tree. Decision made (no-legacy window + types-are-contracts): implement typed tables — the analyzer learns `T` so store methods type as `Result[T, StoreError]`, with generic declared PK types (`String`-friendly; no `Bytes` requirement post-#300). Backing implementation (revive the Ecto path or add a schema check to the ETS store) is C5-internal; C1's registry accounts for typed table shapes from the start — #255.
...
- [#325](https://github.com/kormie/Skein/issues/325) — wire `supervisor` declarations into real OTP supervision *(added 2026-07-02, owner direction: supervision is core to well-controlled agents)*: `skein run` boots declared supervisors with agent children, strategy/intensity from the declaration, restarts visible in the trace, `memory.kv` surviving restarts. Additive on the surface pinned by #319/spec §3.9; belongs with the Wave C "make the declared surface real" honesty work.

**Wave D — Dogfood conformance & release gate** *(1.0 blocker, CONTINUOUS — begins in Wave A, stays green thereafter)* — milestone v0.5.0:
- [#262](https://github.com/kormie/Skein/issues/262) — the gate must **compile + load + RUN** reduced, **pinned** conformance programs from Skein examples, `skein-testing`, and `FablePool-skein`. **Today nothing in Skein compiles, loads, or runs either external repo** (verified: no workflow checks them out; every reference is a comment/changelog citation). The gate must assert real `Result`/error/schema/capability/store shapes and **detect API drift** — both external ports still use ambient `Uuid.new()` (`skein-testing/src/main.skein:114`, `FablePool-skein/src/fablepool.skein:991`), and *(sharpened 2026-07-02)* ambient `Uuid.new()` **no longer exists in the stdlib** (`analyzer.ex:349-350`), so **both ports are known-broken against main today and no gate says so** — the urgency has gone up, not down, since filing; FablePool stubs canonicalization as a string fingerprint (`fablepool.skein:117-120`) and signatures as opaque tags, and uses the surrogate-Uuid `@primary` + `op_id: String @unique` workaround (#255).
- Compile every fenced `skein` block in the spec and docs site ([#202](https://github.com/kormie/Skein/issues/202) subset; `spec_examples_test.exs` covers §8 only today). Snapshot **complete** structured diagnostics, add runtime effect-shape tests for every method (only `http` is shape-tested today), and add property tests where input spaces are wide — including widening the B4 soundness-property generator beyond its current Int/String/Bool module-fn slice ([#314](https://github.com/kormie/Skein/issues/314)). Release-readiness cannot report green without the dogfood jobs.
- Factor-quality companion (sequenced with C1, never before): extract the hand-maintained analyzer registries into the C1 registry, split the contiguous analyzer pass groups into submodules, and give `assert` a real AST node — [#315](https://github.com/kormie/Skein/issues/315).
