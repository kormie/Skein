...
#### Agent-consumed frozen surfaces

The 1.0 freeze is intentionally limited to **machine-consumed surfaces** that agents, CLIs, LSPs, MCP clients, replay tools, release automation, and downstream conformance suites read directly. Each frozen surface must have both (a) a human-readable contract in the docs/spec/stability policy and (b) an executable drift test or freeze vector that fails before a release tag if the contract changes accidentally. Release-readiness treats the inventory below as a manifest: if a row loses its vector/test, the release is blocked until the row is restored or explicitly cut from the freeze.

| Frozen surface | Human-readable contract | Executable drift test / vector |
|---|---|---|
| Grammar accepted at 1.0 | `docs/SKEIN_SPEC.md` §§2–3 describe the frozen syntax, expression-termination rules, scenario surface, and declaration forms. | `apps/skein_compiler/test/skein/parser_test.exs`, `apps/skein_compiler/test/skein/parser_termination_test.exs`, and `apps/skein_compiler/test/skein/conformance/{positive_corpus_test.exs,negative_corpus_test.exs}` pin the accepted/rejected grammar corpus. |
| Reserved + contextual keywords | `docs/SKEIN_SPEC.md` §2.3 and the lexer docs list which words are reserved and which are contextual-only. | `apps/skein_compiler/test/skein/freeze/keywords_freeze_test.exs` against `conformance/freeze/keywords.json`. |
| Diagnostics | `docs/SKEIN_SPEC.md` §7 documents diagnostic codes, categories, severities, structured fields, and edit kinds. | `apps/skein_compiler/test/skein/freeze/diagnostics_freeze_test.exs` against `conformance/freeze/diagnostics.json`. |
| `fix_code` / machine-applicable edits | `docs/SKEIN_SPEC.md` §7 and the agent-writability notes require exact, applicable Skein edits or `nil` — never prose placeholders. | `apps/skein_compiler/test/skein/analyzer_fix_code_test.exs`, `apps/skein_compiler/test/skein/error_edit_test.exs`, and `apps/skein_lsp/test/skein/lsp/code_actions_test.exs`. |
| Effect ABI | `docs/SKEIN_SPEC.md` §6 and `docs/ARCHITECTURE.md` describe the registry-derived signatures, provider contracts, and structured error enums. | `apps/skein_compiler/test/skein/freeze/effect_abi_freeze_test.exs` against `conformance/freeze/effect_abi.json`, plus `apps/skein_runtime/test/skein/runtime/effect_abi_matrix_test.exs`. |
| Schema derivation | `docs/SKEIN_SPEC.md` records the derived JSON Schema contract for record/enum/tool/request surfaces. | `apps/skein_compiler/test/skein/freeze/json_schema_freeze_test.exs` against `conformance/freeze/json_schema_vectors.json`, plus runtime validation coverage in `apps/skein_runtime/test/skein/runtime/json_schema_test.exs`. |
| CLI JSON | CLI reference/help and stability docs describe JSON output versions and machine-readable result envelopes. | `apps/skein_cli/test/cli/cli_surface_freeze_test.exs` against `conformance/freeze/cli_surface.json`. |
| Config keys | `docs/SKEIN_SPEC.md` and the CLI/reference docs describe stable `skein.toml` keys. | `apps/skein_cli/test/cli/cli_surface_freeze_test.exs` against the `config_keys` portion of `conformance/freeze/cli_surface.json`, with behavioral coverage in `apps/skein_cli/test/cli/config_test.exs`. |
| EventStore shapes | `docs/STABILITY.md` and the runtime persistence docs define the persisted event shape compatibility rules. | `apps/skein_runtime/test/skein/runtime/event_store_freeze_test.exs`. |
| Example corpus | The spec/docs examples and dogfood reductions define the examples agents copy, mutate, and use as conformance seeds. | `apps/skein_compiler/test/skein/examples_test.exs`, `apps/skein_compiler/test/skein/spec_examples_test.exs`, `apps/skein_compiler/test/skein/conformance/docs_fences_test.exs`, and `apps/skein_cli/test/cli/dogfood_corpus_test.exs`. |

Release-readiness now has a dedicated **frozen-surface manifest** shell gate, in addition to the tests above, that asserts every row's vector/test file is present before the tag can be cut. This catches checklist drift (for example, documenting a frozen surface without a checked-in vector, or deleting a vector while leaving the surface in the 1.0 promise) even before ExUnit starts.

The freeze does **not** cover non-agent-facing internals: analyzer implementation modules, codegen helper layout, ETS table names, private runtime structs, internal process topology, non-exported dunder helpers, test harness helpers, and other implementation details may continue to change within 1.x as long as the machine-consumed surfaces above remain compatible. Keeping these internals out of the 1.0 promise avoids ossifying refactors that agents do not consume.
- ~~[#332](https://github.com/kormie/Skein/issues/332) — the freeze itself~~ — **landed 2026-07-02**: every frozen surface now has an executable gate against the frozen vectors under `conformance/freeze/` — reserved + contextual keywords (incl. the complete contextual inventory, spec §2.3, two-way drift-tested), the diagnostics registry (spec §7 codes/categories/severities + structured fields + edit kinds + the three-place registry discipline), the full effect ABI (registry dump: signatures, provider contracts, error enums), JSON Schema derivation vectors for canonical declarations, the CLI surface (usage text + zsh completions verbatim, `--json` schema versions, `skein.toml` keys), compiled-metadata classes (dunder inventory + exact entry keys), EventStore persisted event vectors (STABILITY row flipped Pre-stable → Stable), and the dogfood pins (exact SHAs + corpus sync). Status paragraphs flipped in `docs/STABILITY.md`, the docs-site stability page, and the spec banner; release-readiness runs the freeze gates as a mechanical release gate. Deliberate additive changes regenerate a vector with `FREEZE_REGEN=1` in the same PR.
- ~~[#320](https://github.com/kormie/Skein/issues/320) — agent-writability benchmark~~ — **landed 2026-07-02**: `mix skein.bench` runs a 12-task generate-compile-fix loop (`Skein.CLI.Bench`) — generate from the agent primer, compile, mechanically apply machine-applicable fixes (`Skein.Error.Edit.apply_fix/2`), feed structured diagnostics back, iterate to green — reporting first-try compile rate, mean iterations-to-green, and non-converging diagnostics. Replay mode (default) re-runs the checked-in recordings (`conformance/writability/recordings.json`) deterministically: `bench_replay_test.exs` pins it per PR and release-readiness reports the metrics; live runs are on demand (`-- --live`, or the Writability Bench GitHub workflow). First live run: 12/12 green, 9/12 first-try, mean 1.17 iterations — after fixing the convergence-killing diagnostics it exposed ([#336](https://github.com/kormie/Skein/issues/336): Some/None construction steered to a wrong machine-applicable variant replacement, tool-implement `input.<field>` got a shadowing `let input` template, E0029/scenario-body E0001 never mentioned the `expect` block).
- ~~[#334](https://github.com/kormie/Skein/issues/334) — live-backend verification of `llm.stream`~~ — **verified fixed 2026-07-02**: the dogfood timeout (kormie/skein-testing#26) does not reproduce against the live Anthropic backend — the exact repro prompt streams in ~1.3s and a 48s/96-chunk stream completes; the original hang (receive loop matching the `Req.Response.Async` struct instead of its ref) was fixed by the `AsyncBody` rework (#178) and is regression-pinned by the stub SSE tests. Live runs stay manual; CI is stub/replay only.
