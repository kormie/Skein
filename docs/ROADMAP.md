...
- **Remaining (follow-ups):** [#282](https://github.com/kormie/Skein/issues/282) context propagation to spawned processes/tasks/timers (`CapabilityStack`/`TestPolicy` `snapshot`/`restore` exist for the hand-off); `llm.embed` provider support; #284 documented JSON output schemas for check/test/trace.

**Wave 4 — CLI observability & agent contract** *(1.0 blocker)*:
- **Split [PR #239](https://github.com/kormie/Skein/pull/239):** land the framework-neutral pieces first — pure `Skein.CLI.Render` (fix the `skein trace` crash: `main.ex:149` reads `span.name`/`span.duration_ms`, spans carry `:kind`/`:duration_us` — `trace.ex:11-18`), byte-stable plain output, golden trace tests. Then a TTY/TUI seam (`--interactive` opt-in, `--no-tui`/`SKEIN_NO_TUI` force plain; MCP/LSP/non-TTY never route through TUI). JSON output schemas for check/test/trace. The **Raxol TUI** ([#171](https://github.com/kormie/Skein/issues/171)) is **separately gated and may land in 1.0 (Wave 4) if it passes its gates** (binary size ≤ +25%, cold-start delta ≤ +400 ms, no NIF on any target, clean teardown, four-target CI, macOS verification) — otherwise it slips to v1.2. It must never route MCP/LSP/non-TTY output through the TUI. The framework-neutral render/JSON work (#284) is a 1.0 blocker regardless and is not coupled to the TUI.
